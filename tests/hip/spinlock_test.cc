...
__global__ void lock_blockdim_with_critical_manager(Lock *locks, int num_iters, uint32_t *counters)
{
...
int main()
{
  ...
  auto locks = embers::device::allocate_unique<embers::SpinLock<>[]>(dev, num_locks);
  ...
  HIP_CHECK(hipMemset(locks.get(), 0x0, sizeof(embers::SpinLock<>) * num_locks));
  ...
  lock_blockdim_with_critical_manager<embers::SpinLock<>>
